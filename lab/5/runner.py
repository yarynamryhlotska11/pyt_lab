...
if __name__ == '__main__':
    ...
    while True:
        ...
        if choice not in ["1", "2"]:
            print("Error: Incorrect selection. Please choo se the correct option.")
            continue
        length, width = 0, 0
        ...
        if choice in ["1", "2"]:
            ...
            if choice == "2":
                while True:
                    length = input(
                        "Enter the length of the parallelepiped (must be a number greater than or equal to 4): ")
                    width = input(
                        "Enter the width of the parallelepiped (must be a number greater than or equal to 4): ")
                    try:
                        length = int(length)
                        width = int(width)
