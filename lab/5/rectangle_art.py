...
class RectangleArt:
    def __init__(self, width, height, outer_color='BLUE', middle_color='MAGENTA', inner_color='RED', symbol_count=1, symbol_color='*'):
        if width < 1 or height < 1:
            raise ValueError("Error: Rectangle dimensions must be greater than or equal to 1.")
        self.width = width
        self.height = height
        self.outer_rectangle_color = colors.get(outer_color, 'BLUE')
        self.middle_rectangle_color = colors.get(middle_color, 'MAGENTA')
        self.inner_rectangle_color = colors.get(inner_color, 'RED')
